...
const Episode = (data) => {
    ...
    return ( 
        <div>
            <Accordion activeIndex={activeIndex} onTabChange={(e) => setActiveIndex(e.index)} onTabOpen={(e) => openEpSynopsis(e.index)}>
                {
                    dataEpisodes.map(item => 
                        <AccordionTab key={item.mal_id} header={`Episode ${item.mal_id}: ${item.title}`}>
                            {item.synopsis}
                        </AccordionTab>
                    )
                }
